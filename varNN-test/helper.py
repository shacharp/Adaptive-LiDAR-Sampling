...
class logger:
    def __init__(self, args, test, prepare=True):
        self.args = args
        ...
        if test == "yes":
            self.val_csv = os.path.join(output_directory, 'test.csv')
        else:
    ...
    def get_ranking_error(self, result):
        return getattr(result, self.args.rank_metric)  # returns the value of the named attribute of an object. If not found, it returns the default value provided to the function.
...
def get_folder_name(args):
    ...
    if args.use_pose:
        prefix = "var.mode={}.w1={}.w2={}.".format(args.train_mode, args.w1,
                                               args.w2)
    else:
        prefix = "var.test.mode={}.".format(args.train_mode)
    return os.path.join(args.result, prefix + 'input={}.resnet{}.time={}'.format(args.input, args.layers, current_time))
