...
def get_paths_and_transform(split, args):
    ...
    if split == "train":
        ...
        glob_d = os.path.join(
            args.data_folder, 'data_depth_velodyne/train/*_sync/proj_depth/velodyne_raw/image_0[2,3]/*.png')  # on the first phase we will have velodyne in the folder (otherwise we'll have error in the end of this function) but we will not use it in our NN
        if 'final' in args.data_folder:  # final NN
            glob_gt = os.path.join(
                args.data_folder, 'data_depth_annotated/train/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png')
        else:  # in one of the phases
            glob_gt = '../data_new/phase_1/mini_set_' + args.data_folder.split('set_')[1] + '/data_depth_annotated/train/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png'  # These don't need to be change every phase

        def get_rgb_paths(p):
            ps = p.split('/')
            if 'final' in args.data_folder:
                pnew = '/'.join([args.data_folder] + ['data_rgb'] + ps[-6:-4] + ps[-2:-1] + ['data'] + ps[-1:])
            else:
                pnew = '/'.join(['../data_new/phase_1/mini_set_' + args.data_folder.split('set_')[1]] + ['data_rgb'] + ps[-6:-4] + ps[-2:-1] + ['data'] + ps[-1:])
            return pnew
        ...
        if args.val == "full":
            ...
            glob_d = os.path.join(
                args.data_folder, 'data_depth_velodyne/train/*_sync/proj_depth/velodyne_raw/image_0[2,3]/*.png')
            glob_gt = '../data_new/phase_1/mini_set_' + args.data_folder.split('set_')[1] + '/data_depth_annotated/train/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png'  # These don't need to be change every phase
            ...
            if 'final' in args.data_folder:  # final NN
                glob_d = os.path.join(args.data_folder, 'depth_selection/val_selection_cropped/velodyne_raw/*.png')
            else:
                glob_d = '../data_new/phase_' + args.data_folder.split('phase_')[1][0] + '/mini_set_1/depth_selection/val_selection_cropped/velodyne_raw/*.png'  # all of them are always in mini_set_1

            if 'final' in args.data_folder:
                glob_gt = os.path.join(
                    args.data_folder, "depth_selection/val_selection_cropped/groundtruth_depth/*.png")
            else:  # in one of the phases
                glob_gt = '../data_new/phase_1/mini_set_1/depth_selection/val_selection_cropped/groundtruth_depth/*.png'  # These don't need to be change every phase
        ...
        if 'final' in args.data_folder:  # final NN
            glob_d = os.path.join(args.data_folder, 'data_depth_velodyne/test/*_sync/proj_depth/velodyne_raw/image_0[2,3]/*.png')
            glob_gt = os.path.join(args.data_folder, 'data_depth_annotated/test/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png')
        else:
            glob_d = '../data_new/phase_' + args.data_folder.split('phase_')[1][0] + '/mini_set_1/data_depth_velodyne/test/*_sync/proj_depth/velodyne_raw/image_0[2,3]/*.png'
            glob_gt = '../data_new/phase_1/mini_set_1/data_depth_annotated/test/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png'

        def get_rgb_paths(p):
            ps = p.split('/')
            if 'final' in args.data_folder:
                pnew = '/'.join([args.data_folder] + ['data_rgb'] + ps[-6:-4] + ps[-2:-1] + ['data'] + [(ps[-1:][0]).replace("proj_depth_groundtruth_", "")])
            else:
                pnew = '/'.join(['../data_new/phase_1/mini_set_' + args.data_folder.split('set_')[1]] + ['data_rgb'] + ps[-6:-4] + ps[-2:-1] + ['data'] + [(ps[-1:][0]).replace("groundtruth_depth", "image")])
            return pnew
...
def depth_read(filename):
    ...
    if np.max(depth_png) != 0:  # if not all zero map (then the check is useless)
